package tui

import "strings"

type helpRow struct {
	keys string
	desc string
}

type helpSection struct {
	title string
	rows  []helpRow
}

func helpTitle(kind screenKind) string {
	switch kind {
	case screenFleet:
		return "КЛАВИШИ · список хостов"
	case screenDashboard:
		return "КЛАВИШИ · сервер"
	case screenProcesses:
		return "КЛАВИШИ · процессы"
	case screenPorts:
		return "КЛАВИШИ · порты"
	case screenContainers:
		return "КЛАВИШИ · контейнеры"
	case screenHistory:
		return "КЛАВИШИ · история"
	case screenLogs:
		return "КЛАВИШИ · логи"
	default:
		return "КЛАВИШИ"
	}
}

func helpSections(kind screenKind) []helpSection {
	common := helpSection{title: "прочее", rows: []helpRow{
		{"c", "чат с ассистентом"},
		{":", "палитра команд"},
		{"?", "эта справка"},
		// «q» и «ctrl+c» выходят из приложения на любом экране, не только на списке
		// хостов: разбор в navigation.go идёт до экранных обработчиков.
		{"q ctrl+c", "выход из sshmon"},
	}}
	switch kind {
	case screenFleet:
		return []helpSection{
			{title: "движение", rows: []helpRow{
				{"↑↓ j k", "по строкам"},
				{"pgup pgdown", "страницами"},
				{"← →", "свернуть · раскрыть детали"},
				{"tab g", "следующая группа"},
				{"a", "все хосты"},
			}},
			{title: "найти", rows: []helpRow{
				{"/", "поиск по имени, адресу и группе"},
				{"f !", "только проблемные"},
				{"esc", "сбросить фильтры"},
			}},
			{title: "сервер", rows: []helpRow{
				{"enter", "полный экран сервера"},
				{"l", "ящик логов под строкой хоста (в деталях — на весь экран)"},
				{"↑↓ s w space", "в ящике логов: хост · источник · warn+ · пауза"},
				{"ctrl+l", "логи на весь экран"},
				{"p o d", "процессы · порты · контейнеры"},
				{"x", "ssh в терминале"},
				{"r", "обновить детали раскрытой карточки"},
				{"ctrl+r", "переподключить сервер"},
			}},
			{title: "прочее", rows: []helpRow{
				{"v", "боковая панель"},
				{"c :", "чат · палитра команд"},
				{"? q ctrl+c", "эта справка · выход"},
			}},
		}
	case screenDashboard:
		return []helpSection{
			{title: "движение", rows: []helpRow{
				{"tab shift+tab", "фокус панели"},
				{"j k", "прокрутка внутри панели"},
			}},
			{title: "разделы", rows: []helpRow{
				{"p", "процессы"},
				{"o d", "порты · контейнеры"},
				{"l ctrl+l", "логи на весь экран"},
				{"h ctrl+h", "история метрик"},
			}},
			{title: "сервисы", rows: []helpRow{
				{"f", "фильтр юнитов"},
				{"enter", "journal выбранного юнита"},
				{"s", "вернуть системный лог"},
			}},
			{title: "сервер", rows: []helpRow{
				{"r", "обновить данные сейчас"},
				{"x", "ssh в терминале"},
				{"ctrl+r", "переподключить сервер"},
			}},
			{title: "прочее", rows: append(common.rows, helpRow{"esc", "назад к списку"})},
		}
	case screenLogs:
		return []helpSection{
			{title: "фильтры", rows: []helpRow{
				{"/", "фильтр по подстроке"},
				{"n N", "следующее · предыдущее совпадение"},
				{"w", "только warn и выше"},
				{"W", "перебор уровней: all · info · warn · error"},
				{"← → s", "источник: журнал, systemd-юниты, контейнеры"},
			}},
			{title: "строка", rows: []helpRow{
				{"↑↓ j k", "выделенная строка"},
				{"y", "скопировать выделенную строку"},
				{"c", "контекст ±5 строк без фильтра"},
				{"t", "скрыть · показать время"},
			}},
			{title: "поток", rows: []helpRow{
				{"space", "пауза хвоста"},
				{"r", "обновить: перезапустить поток"},
				{"ctrl+r", "переподключить сервер"},
			}},
			{title: "движение", rows: []helpRow{
				{"pgup pgdown", "страницами"},
				{"home end", "в начало · в конец"},
			}},
			// Общие строки здесь не годятся: «c» на этом экране — контекст, а не чат.
			{title: "прочее", rows: []helpRow{
				{"x", "ssh в терминале"},
				{":", "палитра команд"},
				{"?", "эта справка"},
				{"q ctrl+c", "выход из sshmon"},
				{"esc", "назад"},
			}},
		}
	case screenHistory:
		return []helpSection{
			{title: "данные", rows: []helpRow{
				{"1-5", "диапазон времени"},
				{"j k", "метрика"},
				{"h l", "курсор по точкам"},
				{"r", "обновить"},
			}},
			{title: "сервер", rows: []helpRow{
				{"x", "ssh в терминале"},
				{"ctrl+r", "переподключить сервер"},
			}},
			{title: "прочее", rows: append(common.rows, helpRow{"esc", "назад"})},
		}
	default:
		return []helpSection{
			{title: "экран", rows: []helpRow{
				{"", "только чтение, обновляется сам"},
				{"r", "обновить данные сейчас"},
			}},
			{title: "сервер", rows: []helpRow{
				{"x", "ssh в терминале"},
				{"ctrl+r", "переподключить сервер"},
			}},
			{title: "прочее", rows: append(common.rows, helpRow{"esc", "назад"})},
		}
	}
}

func helpText(kind screenKind) string {
	var b strings.Builder
	b.WriteString(titleStyle.Render(helpTitle(kind)))
	for _, section := range helpSections(kind) {
		b.WriteString("\n\n" + titleStyle.Render(section.title))
		for _, row := range section.rows {
			b.WriteString("\n  " + padCell(row.keys, 16) + dimStyle.Render(row.desc))
		}
	}
	b.WriteString("\n\n" + dimStyle.Render("esc / ? — закрыть · в статусбаре живут 3–4 самые частые клавиши"))
	return b.String()
}
