...
//
// Публикация никогда не блокируется на медленном подписчике: если буфер полон,
// самое старое событие выбрасывается в пользу нового. При buffer=1 это значит
// «последний побеждает» — подписчик всегда видит актуальное состояние, но может
// не увидеть промежуточные снимки.
