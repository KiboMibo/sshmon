...
свернуть детали, `tab` — группа, `f` — только проблемные, `v` — боковая панель,
`pgup/pgdown` — страница, `p` — Processes, `o` — Ports, `d` — Docker,
`l` — ящик логов над списком (в нём `↑↓` — хост, `s` — источник).

**Dashboard:** `p` — Processes, `o` — Ports, `d` — Docker, `l` (или `ctrl+l`) — Logs
на весь экран, `h` (или `ctrl+h`) — History; `f` — фильтр systemd-юнитов,
`j/k` — выбор юнита, `enter` — его journal, `s` — вернуться к системному логу,
`r` — обновить данные.
