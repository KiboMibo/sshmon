...
- **Права на конфиг.** `~/.config/sshmon/config.yaml` может содержать SSH-пароль и API-ключ LLM в открытом виде. Держите файл доступным только владельцу: `chmod 600 ~/.config/sshmon/config.yaml`. Если файл читается группой или остальными, sshmon предупреждает при старте.
- **Секреты через окружение.** Чтобы не хранить секреты в файле, задайте `password_env` у сервера и `api_key_env` у LLM — sshmon возьмёт значение из переменной окружения. Предпочитайте ssh-agent или ключ (`key`) вместо пароля.
- **Host-key.** Хост-ключи проверяются по `~/.ssh/known_hosts`. Если файла нет, sshmon один раз предупреждает о риске MITM и продолжает работу; `insecure_host_key: true` осознанно отключает проверку для конкретного сервера.
- **MCP.** MCP-сервер работает только на stdio и без аутентификации — запускайте его как локальный дочерний процесс доверенного агента и не публикуйте наружу.
