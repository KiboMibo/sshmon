...
`~/.ssh/conf.d/*.conf`) и показывает пикер: выберите серверы для мониторинга
(`space` — выбрать, `a` — все, `enter` — сохранить). Имена include-файлов
становятся группами серверов. Результат сохраняется в
